package com.jooper.hellocharts_jooper;
...
public class TempoChartActivity extends Activity {
	...
	public static class PlaceholderFragment extends Fragment {
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			View rootView = inflater.inflate(R.layout.fragment_tempo_chart,
					container, false);
		...
		private void generateTempoData() {
			...
			// auto-generation works only for decimal
			// system. So generate custom axis values for example every 15
			// seconds and set custom labels in format
			// minutes:seconds(00:00), you could do it in formatter but here
			// will be faster.
			List<AxisValue> axisValues = new ArrayList<AxisValue>();
			for (float i = 0; i < tempoRange; i += 0.25f) {
				// I'am translating float to minutes because I don't have data
				// in minutes, if You store some time data
				// you may skip translation.
				axisValues.add(new AxisValue(i)
						.setLabel(formatMinutes(tempoRange - i)));
			}
			...
			// Height axis, this axis need custom formatter that will translate
			// values back to real height values.
			data.setAxisYRight(new Axis().setName("Height [m]")
					.setMaxLabelChars(3)
			...
			// case it will looks better with custom
			// viewport. Set
		...
		/**
		 * Recalculated height values to display on axis. For this example I use
		 * auto-generated height axis so I override only formatAutoValue method.
		 */
		private static class HeightValueFormatter extends
			...
			public HeightValueFormatter(float scale, float sub,
					int decimalDigits) {
				this.scale = scale;
				this.sub = sub;
			...
			public int formatValueForAutoGeneratedAxis(char[] formattedValue,
				...
				return super.formatValueForAutoGeneratedAxis(formattedValue,
						scaledValue, this.decimalDigits);
			}
		}
