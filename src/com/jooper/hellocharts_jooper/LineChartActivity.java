...
public class LineChartActivity extends Activity {
	...
	public static class PlaceholderFragment extends Fragment {
		...
		private void toggleCubic() {
			...
			if (isCubic) {
				// It is good idea to manually set a little higher max viewport
				// for cubic lines because sometimes line
				// go above or below max/min. To do that use Viewport.inest()
				// method and pass negative value as dy
				// parameter or just set top and bottom values manually.
				// In this example I know that Y values are within (0,100) range
				// so I set viewport height range manually
				// to (-5, 105).
				// To make this works during animations you should use
				// Chart.setViewportCalculationEnabled(false) before
				// modifying viewport.
				...
				// You have to set max and current viewports separately.
				// In this case, if I want animation I have to set current
				// viewport first and use animation listener.
		 ...
		 * View.animate()). If you operate on data that was set before you don't
		 * have to call {@link LineChartView#setLineChartData(LineChartData)}
		 * again.
		 */
